...
def get_options(db_name: str = None, db_conf: dict = None, use_pool: bool = True):
    if db_name is None:
...
def execute(
    ...
    :return:
    """
    options = get_options(db_name, db_conf, use_pool)
    return Mysql(**options).execute(sql, args, **kwargs)
...
def execute_many(
    ...
    """
    options = get_options(db_name, db_conf, use_pool)
    return Mysql(**options).execute_many(sql_with_args_list, **kwargs)
