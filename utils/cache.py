...
def set_cache_expire_today(key, value):
    ...
    :return:
    """
    next_date = util.get_delay_date(delay=1)
    today_expire = util.str2time(next_date, fmt="%Y-%m-%d") - util.str2time()
    set(key, value, expire=int(today_expire) + 1)  # 增加1秒的缓冲
